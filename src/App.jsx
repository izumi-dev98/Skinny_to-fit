...
const translations = {
  ...
  my: {
    guideButton: 'ဒီပရောဂျက်ကို အသုံးပြုနည်း', heroLabel: 'နေ့စဉ် စွမ်းအင် စီမံချက်', heroTitle: 'ရည်ရွယ်ချက်ရှိရှိ', heroAccent: 'စားသုံးပါ။',
...
function App() {
  ...
  const [language, setLanguage] = useState(() => localStorage.getItem('skinny-to-fit-language') || 'en')
      ...
      <div className="relative mx-auto max-w-7xl px-5 pb-12 pt-6 sm:px-8 lg:px-12">
        ...
        <section className="grid gap-10 pb-10 pt-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-end lg:pt-20">
          <div className="max-w-xl animate-rise">
            <div className="mb-5 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-[#e56c48]"><span className="h-px w-8 bg-[#e56c48]" /> {t.heroLabel}</div>
            <h1 className="font-display text-5xl font-bold leading-[0.98] tracking-[-0.04em] text-[#26352f] sm:text-7xl">{t.heroTitle} <em className="font-editorial font-normal text-[#e56c48]">{t.heroAccent}</em></h1>
...
function DetailedCalculationBreakdown({ form, result, macros, activity, language, t, onClose }) {
  ...
  return <div className="fixed inset-0 z-[60] flex items-center justify-center overflow-y-auto bg-[#26352f]/55 p-4" role="presentation">
    <section className="my-4 max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[1.75rem] bg-[#fffaf4] p-6 shadow-2xl sm:p-8" role="dialog" aria-modal="true" aria-labelledby="detailed-calculation-title">
      ...
      <button className="button-primary mt-6 w-full" type="button" onClick={onClose}>{t.closeDetails}</button>
    </section>
  </div>
}
